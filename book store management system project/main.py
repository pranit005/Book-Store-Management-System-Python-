...
def cr():
    def create():
        f=open("users.csv","w")
        fwritter=csv.writer(f)
        fwritter.writerow([str(e1.get()),str(e2.get())])
        f.close()
    ...
    Label(n,text="Create Your Account",font=12,bg="#4A148C",fg="white").grid(row=0,column=0,columnspan=2,padx=15,pady=20)

    Label(n,text="Enter Your Name: ",bg="#4A148C",fg="white").grid(row=1,column=0,pady=10,padx=20)
    Label(n,text="Create Password: ",bg="#4A148C",fg="white").grid(row=2,column=0,pady=10,padx=20)
    ...
    Button(n,text="Confirm",width=12,fg="#4A148C",bg="white",command=create).grid(row=3,column=0,columnspan=2,pady=20)
    n.configure(bg="#4A148C")
    n.resizable(height=False,width=False)
    n.mainloop()
...
def chk():
    ...
    for i in fr:
        for j in i:
            data.append(j)
    if data==[]:
        messagebox.showinfo("","please create your account")
    elif data[0]==str(e1.get()) and data[1]==str(e2.get()):
        l.destroy()
        #############################################################################code from here
...
Label(l,text="Login To Book Store Management System",font=12,bg="#4A148C",fg="white").grid(row=0,column=0,columnspan=2,padx=20,pady=30)
Label(l,text="Username: ",bg="#4A148C",fg="white").grid(row=1,column=0,pady=12)
e1=Entry(l)
e1.grid(row=1,column=1)
Label(l,text="Password: ",bg="#4A148C",fg="white").grid(row=2,column=0,pady=12)
e2=Entry(l,show="●")
e2.grid(row=2,column=1)

Button(text="Confirm",width=12,fg="#4A148C",bg="white",command=chk).grid(row=3,column=0,pady=25)
Button(text="New User",width=12,fg="#4A148C",bg="white",command=cr).grid(row=3,column=1,pady=25)



l.configure(bg="#4A148C")
